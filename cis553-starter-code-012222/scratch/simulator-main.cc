...
{
public:
  void Start (std::string scriptFile, NodeContainer nodeContainer, NetDeviceContainer *ndc, uint32_t totalNodes,
              uint32_t totalLinks);
  void Stop ();
  ...
  void Reschedule (std::vector<std::string> tokens, Time time);
  ...
  void SetRoutingVerbose (Ptr<PennRoutingProtocol> routingProtocol, std::vector<std::string> tokens);
  void SetApplicationVerbose (Ptr<PennApplication> application, std::vector<std::string> tokens);
...
void
SimulatorMain::Start (std::string scriptFile, NodeContainer nodeContainer, NetDeviceContainer *ndc, uint32_t totalNodes,
                      uint32_t totalLinks)
{
          ...
          do
            {
              ...
              if (*iterator == "TIME")
                ...
                // If time <= startTime, skip events
                if (time.GetMilliSeconds() < START_TIME)
                {
                  std::cout << "Unable to schedule event at " << time.GetMilliSeconds() << "ms, Application Start Time: " << START_TIME << "ms" << std::endl; 
                }
                else if (time.GetMilliSeconds() == START_TIME)
                {
                  Simulator::Schedule(time, &SimulatorMain::Reschedule, this, m_tokens, time);
                }
                else
                {
...
void
SimulatorMain::Reschedule(std::vector<std::string> tokens, Time time)
{
  Simulator::Schedule (MilliSeconds(0.0), &SimulatorMain::ProcessCommandTokens, this, tokens, time);
}
...
SimulatorMain::ProcessCommandTokens (std::vector<std::string> tokens, Time time)
{
  ...
  // Check for Link Command
      ...
      if (tokens.size () == 2)
        {
          ...
          if (operation == "DOWN")
            {
              Simulator::Schedule (MilliSeconds (time.GetMilliSeconds ()), &SimulatorMain::LinkOperation, this,
                                   linkNumber, false);
              return;
            }
      ...
      else if (tokens.size () == 3)
        {
          ...
          if (operation == "DOWN")
            {
              Simulator::Schedule (MilliSeconds (time.GetMilliSeconds ()), &SimulatorMain::P2POperation, this, nodeANum,
                                   nodeBNum, false);
              return;
            }
          else if (operation == "UP")
  ...
  else if (*iterator == "NODELINKS")
    {
      ...
      if (operation == "DOWN")
        {
          Simulator::Schedule (MilliSeconds (time.GetMilliSeconds ()), &SimulatorMain::AllInterfacesOperation, this,
                               nodeNumber, false);
          return;
        }
      else if (operation == "UP")
      ...
      for (uint32_t i = 0; i < protocols; i++)
        {
          ...
          if (module == LS_MODULE_NAME)
            {
              Ptr<LSRoutingProtocol> lsRouting
                  = DynamicCast<LSRoutingProtocol> (listRouting->GetRoutingProtocol (i, priority));
              if (lsRouting)
                {
                  if (command == "VERBOSITY" || command == "VERBOSE")
                    {
                      ...
                      if (tokens.size () == 0)
                        return;
                      Simulator::Schedule (MilliSeconds (time.GetMilliSeconds ()), &SimulatorMain::SetRoutingVerbose,
                                           this, lsRouting, tokens);
                      return;
                    }
                  else
                    {
                      if (tokens.size () == 2 && strcmp (tokens[0].c_str (), "DUMP") == 0
                          && strcmp (tokens[1].c_str (), "NEIGHBORS") == 0)
                        {
                          Simulator::Schedule (MilliSeconds (time.GetMilliSeconds ()), &startDumpNeighbor);
                          Simulator::Schedule (MilliSeconds (time.GetMilliSeconds ()),
                                               &LSRoutingProtocol::ProcessCommand, lsRouting, tokens);
                          Simulator::Schedule (MilliSeconds (time.GetMilliSeconds ()), &endDumpNeighbor);
                        }
                      else if (tokens.size () == 2 && strcmp (tokens[0].c_str (), "DUMP") == 0
                               && strcmp (tokens[1].c_str (), "ROUTES") == 0)
                        {
            ...
            {
              Ptr<DVRoutingProtocol> dvRouting
                  = DynamicCast<DVRoutingProtocol> (listRouting->GetRoutingProtocol (i, priority));
              if (dvRouting)
                {
                  if (command == "VERBOSITY" || command == "VERBOSE")
                    {
                  ...
                  else
                    {
                      if (tokens.size () == 2 && strcmp (tokens[0].c_str (), "DUMP") == 0
                          && strcmp (tokens[1].c_str (), "NEIGHBORS") == 0)
                        {
                          Simulator::Schedule (MilliSeconds (time.GetMilliSeconds ()), &startDumpNeighbor);
                          Simulator::Schedule (MilliSeconds (time.GetMilliSeconds ()),
                                               &DVRoutingProtocol::ProcessCommand, dvRouting, tokens);
                          Simulator::Schedule (MilliSeconds (time.GetMilliSeconds ()), &endDumpNeighbor);
                        }
                      else if (tokens.size () == 2 && strcmp (tokens[0].c_str (), "DUMP") == 0
                               && strcmp (tokens[1].c_str (), "ROUTES") == 0)
...
void
SimulatorMain::SetApplicationVerbose (Ptr<PennApplication> application, std::vector<std::string> tokens)
{
  if (tokens.size () != 2)
    return;
  std::vector<std::string>::iterator iterator = tokens.begin ();
  ...
  if (type == "TRAFFIC")
    ...
    {
      remove( "output" );
      application->SetOutputFile (on, "output");
    }
  else if (type == "CHORD")
    {
      application->SetChordVerbose (on);
    }
  else if (type == "SEARCH")
    {
      application->SetSearchVerbose (on);
    }
  else if (type == "ALL")
    {
      remove( "output" );
...
void
SimulatorMain::SetRoutingVerbose (Ptr<PennRoutingProtocol> routingProtocol, std::vector<std::string> tokens)
{
  if (tokens.size () != 2)
    return;
  std::vector<std::string>::iterator iterator = tokens.begin ();
  ...
  if (type == "TRAFFIC")
    ...
    {
      remove( "output" );
      routingProtocol->SetOutputFile (on, "output");
    }
  else if (type == "ALL")
    {
      remove( "output" );
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("graderResult", "Name of the grader result checking file", graderResultFile);
  ...
  cmd.AddValue ("project", "Specify which project to run. default is project 1: <1/2>", project);

  cmd.Parse (argc, argv);

  openResultFile (resultFile);
  openGraderResultFile (graderResultFile);

  // Enable Logs
  LogComponentEnable ("SimulatorMain", LOG_LEVEL_ALL);
  LogComponentEnable ("LSRoutingProtocol", LOG_LEVEL_ALL);
  LogComponentEnable ("UdpSocket", LOG_LEVEL_ERROR);
  LogComponentEnable ("TcpSocket", LOG_LEVEL_ERROR);
              ...
              for (uint32_t k = 0; k < protocols; k++)
                {
                  int16_t priority;
                  Ptr<LSRoutingProtocol> lsRouting
                      = DynamicCast<LSRoutingProtocol> (listRouting->GetRoutingProtocol (k, priority));
                  if (lsRouting)
                    {
                  ...
                  Ptr<DVRoutingProtocol> dvRouting
                      = DynamicCast<DVRoutingProtocol> (listRouting->GetRoutingProtocol (k, priority));
                  if (dvRouting)
                    {
                      std::ostringstream nodeId;
                      nodeId << i;
  ...
  if (!animFile.empty ())
    {
      AnimationInterface anim = AnimationInterface (animFile);
    }
